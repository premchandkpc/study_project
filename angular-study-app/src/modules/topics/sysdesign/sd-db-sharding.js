...
// Usage: 4096 IDs/ms per machine, globally unique, time-sortable`,
  ...
  visual: {
    type: 'flow',
    title: '🔀 Database Sharding — Write Path',
    direction: 'horizontal',
    nodes: [
      { id: 'client',  label: 'Client',        color: '#58a6ff', icon: '💻', sublabel: 'App Server' },
      { id: 'router',  label: 'Shard Router',  color: '#ffa657', icon: '🔀', sublabel: 'Consistent Hash' },
      { id: 'shard1',  label: 'Shard 1',       color: '#3fb950', icon: '🗄️', sublabel: 'hash 0–33%' },
      { id: 'shard2',  label: 'Shard 2',       color: '#3fb950', icon: '🗄️', sublabel: 'hash 33–66%' },
      { id: 'shard3',  label: 'Shard 3',       color: '#3fb950', icon: '🗄️', sublabel: 'hash 66–100%' },
      { id: 'replica', label: 'Replica',       color: '#8b949e', icon: '📡', sublabel: 'Async ~10ms lag' }
    ],
    connections: [
      { from: 'client',  to: 'router',  label: 'write(user_id, data)',  protocol: 'TCP' },
      { from: 'router',  to: 'shard1',  label: 'hash → Shard 1',        protocol: 'SQL' },
      { from: 'router',  to: 'shard2',  label: 'hash → Shard 2',        protocol: 'SQL' },
      { from: 'router',  to: 'shard3',  label: 'hash → Shard 3',        protocol: 'SQL' },
      { from: 'shard1',  to: 'replica', label: 'WAL stream',             protocol: 'async' },
      { from: 'shard2',  to: 'replica', label: 'WAL stream',             protocol: 'async' }
    ],
    scenarios: [
      {
        name: '🔢 Range Shard',
        path: ['client', 'router', 'shard1'],
        result: '⚠️ Hotspot risk — N-Z names all hit Shard 2',
        resultColor: '#f85149'
      },
      {
        name: '#️⃣ Hash Shard',
        path: ['client', 'router', 'shard2'],
        result: '✅ Even distribution — but resharding moves 100% of keys',
        resultColor: '#ffa657'
      },
      {
        name: '🔄 Consistent Hash',
        path: ['client', 'router', 'shard3', 'replica'],
        result: '✅ Add node → only ~25% of keys migrate (virtual nodes)',
        resultColor: '#3fb950'
      }
    ]
  },
