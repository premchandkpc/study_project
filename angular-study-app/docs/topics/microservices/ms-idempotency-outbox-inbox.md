...
2. What happens if the outbox relay publishes to Kafka but crashes before marking the row sent?
   Answer: The relay will publish the same event again after restart. That is expected. The event must have a stable event ID, and every consumer must dedupe through an inbox table or idempotent upsert. Outbox gives no-lost-events; inbox/idempotent consumers handle duplicate-events.
