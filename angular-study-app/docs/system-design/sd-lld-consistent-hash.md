...
Tag: `LLD`
Doc path: `docs/system-design/sd-lld-consistent-hash.md`
...
1. How does consistent hashing minimise key remapping when a node is added?
   With mod-N hashing and N -> N+1: key K remaps if `hash(K) % (N+1)  hash(K) % N`. This is true for ~N/(N+1) of all keys -> nearly everything remaps.
