...
Tag: `Protocols`
Doc path: `docs/system-design/sd-protocols-grpc.md`
...
1. How does Protobuf serialisation achieve smaller payload sizes than JSON?
   Protobuf uses field **numbers** (varint-encoded) instead of string field names. Each field is encoded as a tag (field number + wire type) followed by the value - no quotes, no brackets, no whitespace.
...
2. How would you expose a gRPC service to a browser-based frontend?
   Browsers can't call gRPC natively (no HTTP/2 trailer support). Options:
