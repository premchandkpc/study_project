...
Tag: `Case Study`
Doc path: `docs/system-design/sd-case-ticket-booking.md`
...
3. How do you implement a virtual waiting room?
   Redis sorted set: ZADD booking:queue:{event_id} {unix_timestamp} {user_id} with NX flag (no re-join). User sees their position via ZRANK. A background worker runs every second: ZPOPMIN queue 500 -> issue each popped user a time-limited booking token (JWT with exp=10min). If user doesn't use token in 10 minutes -> token expires -> they'd need to re-join queue. The front-end polls GET /queue-position every 5 seconds using long-polling or SSE for live updates. The queue drainer adjusts throughput based on booking server load metrics.
