...
Tag: `Security`
Doc path: `docs/system-design/sd-security-auth.md`
...
- Subject / User / Device: Starts login and presents credentials or device proof.
- Client / Browser / App: Creates code verifier, stores tokens safely, and sends bearer token.
- Identity / IdP / Auth Server: Authenticates user, issues signed tokens, rotates keys, and handles refresh.
- Resource / API / Resource Server: Verifies token, checks scopes/RBAC, and executes authorized request.
- Audit / Audit / SIEM: Records login, denial, key rotation, and suspicious access events.
...
1. How do you revoke a JWT before it expires?
   JWTs are stateless - the server doesn't track them. To revoke:
