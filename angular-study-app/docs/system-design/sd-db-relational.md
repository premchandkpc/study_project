...
Tag: `Database`
Doc path: `docs/system-design/sd-db-relational.md`
...
- Application / App Server: Business logic
- Application / Connection Pool: HikariCP pools DB connections. Creating a DB connection is expensive (~50ms). Pool reuses connections, bounding max concurrent queries.
- Database / Primary (Write): Receives all INSERT/UPDATE/DELETE. Writes WAL to disk before ACK. Streams WAL to replicas.
- Database / Read Replica 1: Streams WAL from primary. Lag is typically <100ms. Use for read-heavy queries, reports, analytics.
- Database / Read Replica 2: Second replica in different availability zone. Automatic failover promoted to primary on primary failure.
- Storage / WAL: Every transaction writes to WAL first. Enables crash recovery and replication. fsync ensures durability.
- Storage / Heap Files: Tables stored as 8KB pages. Indexes stored separately. VACUUM reclaims dead tuples from MVCC.
...
1. When would you denormalize a database schema?
   Normalisation (3NF) eliminates redundancy and ensures consistency. Denormalization intentionally adds redundancy for read performance.
...
2. What causes a sequential scan when an index exists?
   The query planner chooses seq scan when it estimates that reading the index + heap fetches costs MORE than a full table scan. This happens when:
